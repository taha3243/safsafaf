...
app.get("/", (request, response) => {
  //Lord Creative
  console.log(Date.now() + "Sarkozy  7/24 AKTİF TUTMA İŞLEMİ BAŞARILI"); //Lord Creative
...
client.on("guildMemberAdd", async member => {
  ...
  client.channels
    .get(frenzykanal)
    .send(
      `<a:kanltik:779438002803441764> | O Sunucumuza Yeni Biri Geldi Ve İsmi ${member}, Hoşgeldin  **${frenzysayı}** Kişiye Ulaşmak İçin  **${sonuç}** Kişi Kaldı.`
    );
});
client.on("guildMemberRemove", async member => {
  ...
  client.channels
    .get(frenzykanal)
    .send(
      `<a:yapmyorum:779042990995275796> | Olamaz ${member}, Sunucudan Ayrıldı! **${frenzysayı}** Kişiye Ulaşmak İçin  **${sonuç}** Kişi Kaldı.`
    );
  return;
});
